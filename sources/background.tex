...
\begin{tabular}{||c c c||}
\hline
Id & Affected System & Vendor \\ [0.5ex]
\hline
\hline
...
%This makes it harder for attackers to "trick" the kernel into using instructions or data from a user-mode program. 

Also because kennel does need to legally get data from user space, two more new instructions are provided to accomplish that. "STAC"/"CLAC" are to disable/enable SMAP temporarily. The idea is that the kernel should be aware of when it's doing so. 

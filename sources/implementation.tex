...
We need to filter out the irrelevant exceptions first. For example, P:0 in the error code indicates the page absence, and we must pass it to the original page fault handler without any modification. We have not observed a SMAP exception on the invalid page, even though the error code may have combined bits. Other than that, U/S:1 shows it is user-mode access, which will not cause SMAP exception.  The Current Privilege Level (CPL) in the CS segment register also reveals the context. Additionally, we keep track of the protected or relevant pages, which helps to identify the exceptions.



After filtering, ~\autoref{algo:pagefaulthandler} shows the basic algorithm to handle the SMAP and related exceptions.
...
\textbf{\textit{Enable Interrut.}} The page fault exception is called through an Interrupt Gate~\cite{intelinterrupt}. Therefore, when entering the page fault handler, the CPU clears EFLAGS:IF automatically prevents subsequent interrupts from interfering with the handler's execution. Since we need to walk through the page table, it may access an invalid page, which triggers a nested page fault exception to bring the page back from disk. Therefore we need to set EFLAGS:IF before the page table walk.
...
We do not use a bare-metal hypervisor because we create only one virtual machine, the current system, and do not need to emulate hardware. We use multiple VMCS that each represent one logical processor of the physical CPU and loads the logical processor's run-time state. Essentially, each processor exists in the system has its VMCS, hence no need to reload VMCS pointers.
...
To monitor the process context switch, we need the virtual machine exit on control register operations. In the event handler, the hypervisor can modify CPU state through Guest-state area, in this case, it is CR4.SMAP. After that, the hypervisor invokes VMRESUME to re-enter the virtual machine.

\subsection{Updating PTE}

We modify the bits in PTE to change a page's attribute. Updating system metadata such as the page table needs synchronization with other system components. Typically, the modifier should hold a global lock first. However, as a third-party kernel module, we do not have such information on Windows internal data.
...
In a typical kernel-level TOCTOU attacking scenario, the attacker races with the kernel at full speed, so will our mitigation triggered. We modify the page table rapidly. Hence we need to make our operation as atomic as possible. ~\autoref{fig:ptestruct} shows the PTE data structure defined in C on a 32-bit system. The C code at \texttt{\textcircled{1}} changes the Owner a.k.a U/S bit to zero, which makes it a kernel-mode page. \texttt{\textcircled{2}} is the assembly code generated by a compiler, where once C statement needs three assembly instructions. However, during the three instructions, the current thread may be interrupted, or other threads may also update the same PTE due to the race condition, which occurs multiple times when we test the exploit agast \name. We use the locked atomic instruction~\cite{intelmanualchapter8} instead of the compiler-generated assembly code, as shown at \texttt{\textcircled{3}}. It guarantees that the update on one PTE is intact. 
...
In this project, we find a Windows kernel internal function, \texttt{KeFlushSingleTb()}. It sends IPI to all processors to invalid a particular TLB entry, as mentioned above. The hardware abstraction layer (HAL) eventually sends the IPI, so the kernel does not need to mind the actual hardware differences.

\subsection{Intercepting System Calls}

In the user read scenario, \name changes the protected page back to user-mode with a read-only permit. To prevent user threads from change page permits through APIs, we need to intercept system calls such as \texttt{NtProtectVirtualMemory()}, corresponding to the upper-layer Win32 API \texttt{VirtualProtect()}. To go around this system call protection, the attacker may find different ways, which we need to improve. It becomes another arms race between the attacker and the defender, which is out of this paper's scope.

...
%Suppose, at the same time, another thread try to update the same memory location but with a different bit (with slightly different mask in code line 3). Thread A first executes at line 3, the memory data has been read into ecx and then updated by the "and" instruction. Meanwhile, thread B executes at line 2 when the ecx register holds the old value. Next, thread A executes at line 4 and write the memory with the updated value. Then thread B executes at line 3 and line 4, writing an out dated value into memory.
